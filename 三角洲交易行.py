...
import tempfile
import shutil
import pytesseract  # 首先导入 pytesseract

# 判断是否在 PyInstaller 打包环境中运行
if getattr(sys, 'frozen', False):
    # 创建临时目录存放 Tesseract
    tesseract_temp_dir = tempfile.mkdtemp()
    # 复制打包的资源文件到临时目录
    tessdata_src = os.path.join(sys._MEIPASS, 'tesseract', 'tessdata')
    tessdata_dest = os.path.join(tesseract_temp_dir, 'tessdata')
    shutil.copytree(tessdata_src, tessdata_dest)
    
    # 设置 Tesseract 路径
    os.environ['TESSDATA_PREFIX'] = tessdata_dest
    tesseract_cmd = os.path.join(sys._MEIPASS, 'tesseract', 'tesseract.exe')
...
class AutoTraderGUI:
    def __init__(self, root):
        self.root = root
        root.title("游戏交易行监控助手 v3.2 基于b站文真则v2.1(github.com/wenzhenze)修改版")
...
if __name__ == "__main__":
    try:
        ...
        with open("error.log", "w") as f:
            ...
            f.write(f"系统路径: {sys.path}\n")
            f.write(f"TESSDATA_PREFIX: {os.environ.get('TESSDATA_PREFIX', '未设置')}\n")
            f.write(f"tesseract_cmd: {pytesseract.pytesseract.tesseract_cmd}\n")
        
        # 显示错误对话框
        error_msg = f"程序发生错误: {str(e)}\n详细信息已保存到 error.log"
        tk.messagebox.showerror("致命错误", error_msg)
